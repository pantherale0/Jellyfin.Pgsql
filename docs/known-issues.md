...
| Live TV Multiview | Experimental web overlay ([`jellyfin_web_zzz_livetv_multiview`](patches.md#jellyfin_web_zzz_livetv_multiviewpatch)): each tile is a separate Live TV open + `hls.js` player (no per-tile subtitle/bitrate controls). Four simultaneous streams can exhaust tuner/transcode capacity. Fully disabled on webOS / TV via `isTvClient()` (setting forced off, UI hidden, `open()` no-op). `EnableLiveTvMultiview` / `Policies.LiveTvMultiview` gates the UI only—LiveStreams APIs are unchanged. Existing users without a permission row need an admin to enable Multiview (`HasPermission` defaults false); new users default enabled. |
| Transcode codec parallel race | When [`EnableParallelCodecRace`](patches.md#jellyfin_z_transcode_codec_fallbackpatch) is on, HLS init may launch two full ffmpeg jobs (AV1 + H.264) briefly — expect doubled GPU/CPU load during startup until one wins. Cap is two concurrent races server-wide. HLS fmp4 init path only (not DASH/progressive in v1). |
| AV1 hardware encode | `AllowAv1Encoding` + NVENC still requires Ada-or-newer for reliable AV1 **encode**; pre-Ada GPUs may list `av1_nvenc` but fail at runtime. Fallback/race mitigates playback failures; software AV1 remains last in the sequential chain. See [`jellyfin_z_transcode_codec_fallback`](patches.md#jellyfin_z_transcode_codec_fallbackpatch). |
...
| `AddUserTasteBecauseYouRecommendations` `42P07` already exists | A prior image or failed startup can leave `UserTasteBecauseYouRecommendations` in PostgreSQL without a matching `__EFMigrationsHistory` row; the next boot then fails `CREATE TABLE`, restores the pre-migration dump, and loops. The migration now uses `CREATE TABLE IF NOT EXISTS` / `CREATE INDEX IF NOT EXISTS`. Re-pull the image and restart once. |
| Active-standby HA | Off by default (`Pgsql_HA_ENABLED`). `/config` is still single-writer (share `JELLYFIN_SERVER_ID` if pods do not share `device.txt`). In-flight TCP/HLS/Live TV sockets die; Live TV needs a client restart (`LiveStreamFenced` — web auto-retries once; see [`jellyfin_web_zzzz_playback_errors`](patches.md#jellyfin_web_zzzz_playback_errorspatch)). Sidecar Redis on localhost cannot elect or overlay progress across pods — extract Redis first. When enabling HA, switch k8s readiness from `/health` to `/health/ready`. |
| Playback error messaging (mobile) | Jellyfin Android / Swiftfin / other clients do not yet consume `X-Playback-Error-Code` or the extended `PlaybackErrorCode` enum — they may still show generic errors for mid-stream failures until updated. Web uses [`jellyfin_web_zzzz_playback_errors`](patches.md#jellyfin_web_zzzz_playback_errorspatch). Contract: [features → Playback error messaging](features.md#playback-error-messaging). |
...
| Web UI blank / `getDefaultLayout` TypeError | [`jellyfin_web_zzzz_playback_errors`](patches.md#jellyfin_web_zzzz_playback_errorspatch) imported `playbackError` from `htmlMediaHelper` (loaded via `apphost`), creating a circular import through `dialog` → `layoutManager` → `apphost` before `appHost` was defined. Fixed by moving HLS error header parsing to `playbackErrorCode.ts` and lazy-loading `appRouter` in `showPlaybackError`. Re-pull the image after this patch update. |
| Transcode codec fallback / web retry | With [`EnableTranscodeCodecFallback`](patches.md#jellyfin_z_transcode_codec_fallbackpatch), sequential fallback could dispose the live job's `CancellationTokenSource` while ffmpeg was still running, so playback stopped in the web UI while orphan transcodes accumulated and `POST /Sessions/Playing/Stopped` threw `ObjectDisposedException`. Fixed in [`jellyfin_z_transcode_codec_fallback`](patches.md#jellyfin_z_transcode_codec_fallbackpatch) (keep CTS on success, kill failed attempts) and [`jellyfin_web_zzzz_playback_errors`](patches.md#jellyfin_web_zzzz_playback_errorspatch) (`stop()` before playback retry). Re-pull the image after this patch update. |
| webOS play-start lag / frozen OSD | On LG webOS, the stock multi-layer MDL spinner plus early `Screenfull.request` compete with decode, so video can start while the loader lags and controls feel frozen when the OSD mounts. Mitigated by [`jellyfin_web_zzzz_tv_playback_perf`](patches.md#jellyfin_web_zzzz_tv_playback_perfpatch) (lightweight TV spinner + deferred fullscreen) and Multiview disabled + lazy-import on webOS/TV in [`jellyfin_web_zzz_livetv_multiview`](patches.md#jellyfin_web_zzz_livetv_multiviewpatch). Residual TV jank may remain on very low-end Chromium 68 devices. |
